...
const Cart = () => {
  ...
  const removeFromCartAndDB = async (item) => {
    const cartDocId = item._id;

    removeFromCart(cartDocId);

    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/cart/${cartDocId}`,
        { method: 'DELETE' },
      );
            ...
            <div className="row">
                        ...
                        <tr key={item._id}>
                          ...
                          <td width="20%">
                            <QuantityBox
                              ...
                              setQuantity={(newQty) =>
                                updateQuantityInCart(item, newQty)
                              }
                            />
                          </td>
                          ...
                          <td width="10%">
                            <span
                              className="remove"
                              onClick={() => removeFromCartAndDB(item)}
                            >
                              <IoIosClose />
                            </span>
                          </td>
              ...
              <div className="col-md-3">
                <div className="card border p-3 cartDetails">
                  ...
                  <Link to={'/checkout'}>
                    <Button className="btn-blue btn-lg btn-big bg-red">
                      <IoBagCheckOutline />
                      &nbsp; Checkout
                    </Button>
                  </Link>
